        ...
        if msg_type == 'text':
            if state['step'] == 'start':
                ...
                reply_text = (
                    ...
                    f"この構成でスプレッドシートを作成してよろしいですか？「はい」「いいえ」でお答えください。"
                )
                ...
                if "はい" in user_message:
                    ...
                    reply(reply_token, "ありがとうございます。予約表の記録フォーマットを作成します。")
                    reply(reply_token, "予約表のデータ取得を完了しました！")
                    reply(reply_token, (
                        "📷 予約状況の連絡方法：\n\n"
                        "① 紙の予約表の写真をそのまま送信 → AIが自動で更新\n\n"
                        "② または、以下のように送ってください：\n"
                        "例：「18:30〜、2名、名前：田中様、電話番号：090-xxxx-xxxx」\n\n"
                        "変更・キャンセルもこの方法でOKです。"
                    ))
                    return
                elif "いいえ" in user_message:
            ...
            if state.get("step") == "wait_for_image":
                reply_text = (
                    "予約表の画像を解析しました！\n例：18:00〜、18:30〜、名前と人数の欄があります。\n問題なければ「はい」、修正があれば「いいえ」で返信してください。"
                )
                user_state[user_id]["step"] = "confirm_structure"
            else:
                reply_text = "現在は画像受付の段階ではありません。まず店舗登録を完了してください。"
        else:
            reply_text = "テキストまたは画像を送ってください。"
