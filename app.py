...
if not (GEMINI_API_KEY and LINE_CHANNEL_ACCESS_TOKEN and GOOGLE_JSON):
    raise RuntimeError("GEMINI_API_KEY / LINE_CHANNEL_ACCESS_TOKEN / GOOGLE_CREDENTIALS_JSON が不足しています")
...
def create_store_sheet(name: str, store_id: int, seat_info: str, times: List[str]) -> str:
    sh  = gs_client.create(f"予約表 - {name} ({store_id})")
    sh.share(None, perm_type="anyone", role="writer")  # 要調整
    ws  = sh.sheet1
...
def append_reservations(sheet_url: str, rows: List[Dict[str, Any]]):
    ...
    existing = {ws.cell(r, idx).value: r for r in range(2, ws.row_count + 1) if ws.cell(r, idx).value}
    for r in rows:
...
# 4. Gemini ラッパー
# -------------------------------------------------------------
MODEL_TEXT   = "gemini-1.0-pro-latest"
MODEL_VISION = "gemini-1.0-pro-vision-latest"
...
def _gemini_vision(img_b64: str, prompt: str, max_tokens: int = 1024) -> str:
    try:
...
def _extract_times(img: bytes) -> List[str]:
    task = ("画像は空欄の飲食店予約表です。予約可能な時間帯 (HH:MM) を、"
            "左上→右下の順に重複なく昇順で JSON 配列として返してください。")
    res = _gemini_vision(base64.b64encode(img).decode(), task, 256)
    try:
        data = json.loads(res)
...
def _extract_rows(img: bytes) -> List[Dict[str, Any]]:
    task = ("画像は手書きの予約表です。各行の予約情報を JSON 配列で返してください。"
            "形式: [{\"month\":int,\"day\":int,\"time\":\"HH:MM\",\"name\":str,\"size\":int,\"note\":str}]")
    res = _gemini_vision(base64.b64encode(img).decode(), task, 2048)
    try:
        data = json.loads(res)
...
def _download_image(msg_id: str) -> bytes:
...
def _process_template(uid: str, msg_id: str):
    ...
    if not st or st["step"] != "wait_template_img":
        return
    img   = _download_image(msg_id)
    times = _extract_times(img)
    if not times:
        _line_push(uid, "画像の解析に失敗しました。鮮明な『空欄の予約表』を再度お送りください。")
        return
    st["times"] = times
...
def _process_filled(uid: str, msg_id: str):
    ...
    if not rows:
        _line_push(uid, "予約情報が検出できませんでした。もう一度鮮明な画像をお願いします。")
        return
...
def _handle_event(event: Dict[str, Any]):
    try:
        ...
        if mtype == "text":
            ...
            # ③ 座席数抽出
            if step == "ask_seats":
                seat_prompt = (f"以下の文から 1人席, 2人席, 4人席 の数を抽出し、"
                               f"次の形式で出力してください:\n1人席: ◯席\n2人席: ◯席\n4人席: ◯席\n\n{text}")
                seat_info = _gemini_text(seat_prompt, 128)
                if not seat_info:
            ...
            if step == "confirm_seats":
                if "はい" in text:
                    ...
                    _line_reply(token,
                        ...
                        "まず『空欄の予約表』の写真を送ってください。AI がフォーマットを学習します。")
                elif "いいえ" in text:
                    st["step"] = "ask_seats"
                    _line_reply(token, "わかりました。座席数をもう一度入力してください。")
                else:
                    _line_reply(token, "「はい」または「いいえ」でお答えください。")
                return
            ...
            if step == "confirm_times":
                if "はい" in text:
                    ...
                    _line_reply(token,
                        "スプレッドシートを作成しました！\n"
                        f"📄 {st['sheet_url']}\n\n"
                        "当日の予約を書き込んだ紙の写真を送ると、自動で追記します。")
                elif "いいえ" in text:
                    st["step"] = "wait_template_img"
                    _line_reply(token, "わかりました。空欄の予約表画像をもう一度お送りください。")
                else:
                    _line_reply(token, "「はい」または「いいえ」でお答えください。")
                return
