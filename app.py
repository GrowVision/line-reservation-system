...
def create_store_sheet(name: str, store_id: int, seat_info: str, times: List[str]) -> str:
    ...
    sh.share(None, perm_type="anyone", role="writer")  # 必要に応じて調整
    ws = sh.sheet1
    ws.update([["月", "日", "時間帯", "名前", "人数", "備考"]])
    if times:
        ws.append_rows([["", "", t, "", "", ""] for t in times], value_input_option="USER_ENTERED")
    _get_master_ws().append_row([name, store_id, seat_info.replace("\n", " "),
...
# -------------------------------------------------------------
# 4. Gemini Vision ユーティリティ
# -------------------------------------------------------------
def _vision_request(img_b64: str, prompt: str, max_tokens: int = 1024) -> str:
...
def _vision_extract_times(img: bytes) -> List[str]:
    ...
    times = _vision_request(base64.b64encode(img).decode(), task, 256)
    try:
...
def _vision_extract_rows(img: bytes) -> List[Dict[str, Any]]:
    ...
    rows = _vision_request(base64.b64encode(img).decode(), task, 2048)
    try:
...
def _download_line_image(msg_id: str) -> bytes:
...
def _process_template(uid: str, msg_id: str):
    ...
    if not st or st["step"] != "wait_template_img":
        return
    img = _download_line_image(msg_id)
    times = _vision_extract_times(img)
    if not times:
        _line_push(uid, "画像の解析に失敗しました。鮮明な『空欄の予約表』をもう一度お送りください。")
        return
    st["times"] = times
...
def _handle_event(event: Dict[str, Any]):
    try:
        ...
        if msg_type == "text":
            ...
            # ③ 座席数入力 --------------------------------------------------
            if step == "ask_seats":
                prompt = (f"以下の文から 1人席, 2人席, 4人席 の数を抽出し、"
                          f'次の形式で出力してください:\n1人席: ◯席\n2人席: ◯席\n4人席: ◯席\n\n{text}')
                seat_info = genai.GenerativeModel("gemini-1.0-pro") \
                                 .generate_content(prompt) \
                                 .text.strip()
